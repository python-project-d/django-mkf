...
(function($) {
	$.fn.nc_region = function(options) {
		...
		function initArea($inputArea) {
			settings.$area = $("<select class=ssq></select>");
			$inputArea.before(settings.$area);
				var ids=$inputArea.attr("ids");
				if(ids==undefined){
					loadAreaArray(function() {
						loadArea(settings.$area, settings.area_id)
					})
				}else{
					loadAreaArray1(function() {
						loadArea(settings.$area, settings.area_id)
					})
				}
		}
		...
		function loadAreaArray1(callback) {
			$.getJSON(SITEURL + "/index.php?act=index&mendian=1&op=json_area&src=" + settings.src + "&callback=?", function(data) {
				nc_a = data;
				callback()
			})
		}
...
/* 加入购物车 */
function addcart(goods_id,quantity,callbackfunc,service) {
    if (!quantity) return false;
    var url = 'index.php?act=cart&op=add';
    quantity = parseInt(quantity);
    $.getJSON(url, {'goods_id':goods_id,'service_id':service ,'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
            	if(callbackfunc){
...
//领取红包
function conpen(conpen_id){
    $.get('index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();

        }else{
            var url = 'index.php?act=redpacket&op=getredpacketsave&tid='+conpen_id;
...
//领取优惠卷
function redpacket(conpen_id){
    $.get('index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?act=voucher&op=getvouchersave&tid='+conpen_id;
            ajaxget(url);
        }
    });
}
function voucher_canget(yprefix,yclassname){ //yprefix代金卷id前缀，yclassname代金卷失效或已领样式
	$.get(SHOP_SITE_URL+"/index.php?act=pointvoucher&op=voucher_can_get", function(data){
		var tbody = "";  
		var did='#'+yprefix
		var obj = eval("("+data+")");  
		$.each(obj, function (n, value) {  
			if(value.canget==0){
				  $(did+n).addClass(yclassname);  
			}
		});  
	});
}
function show_voucher_bar(){
	$.get('index.php?act=index&op=login', function(result){
		if(result=='0'){
			login_dialog();
		}else{
		   ...
		   $("#content-djj").animate({right:'35px'});	
		   $("#djjlist").load('index.php?act=goods&op=youhuijuan_min');
	   }
    });	
}
function goods_voucher_get(tid){ //add by duruijin
    $.get('index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
			ajaxget('index.php?act=voucher&op=getvouchersave&jump=0&tid='+tid);
			$("#djjlist").load('index.php?act=goods&op=youhuijuan_min');
        }
    });	
}
$(document).ready(function(){   
    ...
    $(document).click(function(e){  
		if(dwidth-e.pageX>210){
		   $('#content-cart').animate({'right': '-210px'});
		   $('#content-djj').animate({'right': '-210px'});
		   //$("#content-compare").animate({'right':'-210px'});
		}
    });  
});  