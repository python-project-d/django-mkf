...
*/
	...
	var friend_list = new Array();//售前服务
	var recent_list = new Array();//最近联系人
	var shouhou_list = new Array();//售后服务
    ...
    $(function(){
		...
		if(user['u_id'] != '') {
			...
			web_info['msg_dialog'] = '<div id="new_msg_dialog" class="msg-windows"><div class="user-tab-bar"><ul class="user-list" id="user_list"></ul></div>'+
									...
									'<label id="photoBtn" class="feature-btn" title="发送图片"> <i class="icon icon-photo"></i><input id="photoFile" name="photoFile" type="file"  accept="image/gif, image/jpeg, image/jpg, image/png" /> </label> '+
			...
			var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>'+
									...
									'<span class="hide"></span>售前客服</dt><dd id="chat_friends" style="display: none;"></dd></dl>'+
									'<div id="chat_user_shouhou" class="chat-list-content shopnc"><div><dl id="chat_user_shouhou"><dt onclick="chat_show_user_list(\'shouhou\');">'+
									'<span class="hide"></span>售后客服</dt><dd id="chat_shouhou" style="display: none;"></dd></dl>'+
									'<dl id="chat_user_recent"><dt onclick="chat_show_user_list(\'recent\');"><span class="hide"></span>最近联系人</dt><dd id="chat_recent" style="display: none;"></dd></dl></div></div>'+
									'</div></div>';
			...
			$.ajax({
			    ...
			    success: function(u_list){
			  	for (var u_id in u_list){
					...
					if(user_info['shouhou']==1) shouhou_list[u_id]=user_info;
					}
	...
	$.getScript(RESOURCE_SITE_URL+'/js/ajaxfileupload.js');
		$(document ).on('change','#photoFile' , function(){ uploadpic() });
		function uploadpic(){
			$.ajaxFileUpload
			(
				{
					url: CHAT_SITE_URL+'/index.php?act=web_chat&op=uploadpic', //用于文件上传的服务器端请求地址
	...
	function send_img(img){
		...
		var cid="#onoff"+user_show;
		var onoff=($(cid).attr('class'));
		if(onoff=='offline') {
			$("#send_alert").html(msg['t_name']+'离线状态,系统自动回复');
			msg['offline']=1;
		}else{
			msg['offline']=0;	
		}
	...
	function show_msg(u_id){//显示会员的消息
		...
		var ajaxurl = CHAT_SITE_URL+'/index.php?act=web_chat&prand='+prand+'&op=get_recent&t=member&u_id='+u_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			dataType:"jsonp",
	...
	function show_f_msg_auto(msg){//显示收到的消息
		var u_id = msg['t_id'];
		var text_append = '';
		var obj_msg = obj.find("div[select_user_msg='"+u_id+"']");
		text_append += '<div class="from_msg" m_id="'+msg['m_id']+'">';
		text_append += '<span class="user-avatar"><img src="'+user_list[u_id]['avatar']+'"></span>';
		text_append += '<dl><dt class="from-msg-time">';
		text_append += msg['add_time']+'</dt>';
		text_append += '<dd class="from-msg-text">'+msg['t_name']+'不在线</dd>';
		text_append += '<dd class="arrow"></dd>';
		text_append += '</dl>';
		text_append += '</div>';
		obj_msg.append(text_append);
		get_goods_info(msg);
		obj_msg.append('<div onclick="xiaomi();" class="clear_msg"><a href="Javascript: void(0);">联系迈客小蜜</a></div>');
		obj.find("#msg_list").scrollTop(obj_msg.height());
	}
	function xiaomi(){
		layer.open({
		  ...
		  shade: 0.8,
		  area: ['980px', '700px'],
		  content: SHOP_SITE_URL+'/index.php?act=index&op=xiaomi'
		});	
	}
	...
	function update_chat_msg(msg){
		if(isimg==1){
			return '<a href="'+msg+'" target="_blank"><img src="'+msg+'" width="180"></a>';
		}else{
			...
			if(obj_friend.parent().find("dd[u_id='"+u_id+"']").size()==0) {
				if(user_list[u_id]['online'] > 0 ) {
					obj_friend.before('<dd u_id="'+u_id+'" onclick="chat('+u_id+');"><span class="user-avatar"><img alt="'+user_list[u_id]['u_name']
						+'" src="'+user_list[u_id]['avatar']+'"><i class="online" id="onoff'+u_id+'"></i></span><h5>'+user_list[u_id]['u_name']+'</h5><a href="javascript:void(0)"></a></dd>');
				} else {
					obj_friend.after('<dd u_id="'+u_id+'" onclick="chat('+u_id+');"><span class="user-avatar"><img alt="'+user_list[u_id]['u_name']
						+'" src="'+user_list[u_id]['avatar']+'"><i class="offline" id="onoff'+u_id+'"></i></span><h5>'+user_list[u_id]['u_name']+'</h5><a href="javascript:void(0)"></a></dd>');
				}
			}
		}
		obj_friend.remove();
	...
	function update_shouhou(){
		var obj_shouhou = $("#chat_shouhou");
		for (var u_id in shouhou_list){
			if(obj_shouhou.parent().find("dd[u_id='"+u_id+"']").size()==0) {
				if(user_list[u_id]['online'] > 0 ) {
					obj_shouhou.before('<dd u_id="'+u_id+'" onclick="chat('+u_id+');"><span class="user-avatar"><img alt="'+user_list[u_id]['u_name']
						+'" src="'+user_list[u_id]['avatar']+'"><i class="online" id="onoff'+u_id+'"></i></span><h5>'+user_list[u_id]['u_name']+'</h5><a href="javascript:void(0)"></a></dd>');
				} else {
					obj_shouhou.after('<dd u_id="'+u_id+'" onclick="chat('+u_id+');"><span class="user-avatar"><img alt="'+user_list[u_id]['u_name']
						+'" src="'+user_list[u_id]['avatar']+'"><i class="offline" id="onoff'+u_id+'"></i></span><h5>'+user_list[u_id]['u_name']+'</h5><a href="javascript:void(0)"></a></dd>');
				}
			}
		}
		obj_shouhou.remove();
		chat_show_user_list('shouhou');
	}
