...
function loadCompare(isrefresh){
	...
	if(isrefresh == true){
		$("#comparelist").load(SHOP_SITE_URL+ '/index.php?act=compare&op=showcompare');
	}
}
//添加对比商品
function addCompare(gid){
	...
	if (gid > 0){
       	$.ajax({
            type:"GET",
            dataType: "json",
            url:SHOP_SITE_URL+ '/index.php?act=compare&op=addcompare&id='+gid,  
            async:false,
...
function delCompare(gid,type){	
	$.ajax({
        type:"GET",
        dataType: "json",
        url:SHOP_SITE_URL+ '/index.php?act=compare&op=delcompare&gid='+gid+'&type='+type,  
        async:false,
        success: function(data){  
            if(data.done == true) {
            	//将对比按钮置为未对比状态
...
function initCompare(){
	...
	//根据是否已加入对比，显示不同样式
	$.getJSON(SHOP_SITE_URL+ '/index.php?act=compare&op=checkcompare', function(data){
        if(data){
